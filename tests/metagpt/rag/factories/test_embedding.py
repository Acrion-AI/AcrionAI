...
from acrionai.configs.embedding_config import EmbeddingType
from acrionai.configs.llm_config import LLMType
from acrionai.rag.factories.embedding import RAGEmbeddingFactory
...
class TestRAGEmbeddingFactory:
    ...
    def mock_config(self, mocker):
        return mocker.patch("acrionai.rag.factories.embedding.config")

    @staticmethod
    def mock_openai_embedding(mocker):
        return mocker.patch("acrionai.rag.factories.embedding.OpenAIEmbedding")

    @staticmethod
    def mock_azure_embedding(mocker):
        return mocker.patch("acrionai.rag.factories.embedding.AzureOpenAIEmbedding")

    @staticmethod
    def mock_gemini_embedding(mocker):
        return mocker.patch("acrionai.rag.factories.embedding.GeminiEmbedding")

    @staticmethod
    def mock_ollama_embedding(mocker):
        return mocker.patch("acrionai.rag.factories.embedding.OllamaEmbedding")
