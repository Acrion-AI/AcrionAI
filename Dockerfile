...
# Install Debian software needed by AcrionAI and clean up in one RUN command to reduce image size
RUN apt update &&\
...
ENV CHROME_BIN="/usr/bin/chromium" \
    puppeteer_config="/app/acrionai/config/puppeteer-config.json"\
    PUPPETEER_SKIP_CHROMIUM_DOWNLOAD="true"
RUN npm install -g @mermaid-js/mermaid-cli &&\
    npm cache clean --force

# Install Python dependencies and install AcrionAI
COPY . /app/acrionai
WORKDIR /app/acrionai
RUN mkdir workspace &&\
