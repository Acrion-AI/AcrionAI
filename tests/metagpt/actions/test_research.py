...
from acrionai.actions import research
...
async def test_collect_links(mocker, search_engine_mocker, context):
    async def mock_llm_ask(self, prompt: str, system_msgs):
        if "Please provide up to 2 necessary keywords" in prompt:
            ...
            return (
                '["AcrionAI use cases", "The roadmap of AcrionAI", '
                '"The function of AcrionAI", "What llm AcrionAI support"]'
            )
    ...
    ).run("The application of AcrionAI")
    for i in ["AcrionAI use cases", "The roadmap of AcrionAI", "The function of AcrionAI", "What llm AcrionAI support"]:
        assert i in resp
...
async def test_collect_links_with_rank_func(mocker, search_engine_mocker, context):
    ...
    mocker.patch("acrionai.provider.base_llm.BaseLLM.aask", mock_collect_links_llm_ask)
    resp = await research.CollectLinks(
...
async def test_conduct_research(mocker, context):
    ...
    mocker.patch("acrionai.provider.base_llm.BaseLLM.aask", mock_llm_ask)
    content = (
        "AcrionAI takes a one line requirement as input and "
        "outputs user stories / competitive analysis / requirements / data structures / APIs / documents, etc."
    )

    resp = await research.ConductResearch(context=context).run("The application of AcrionAI", content)
    assert resp == data
...
async def mock_collect_links_llm_ask(self, prompt: str, system_msgs):
    if "Please provide up to 2 necessary keywords" in prompt:
        ...
        return (
            '["AcrionAI use cases", "The roadmap of AcrionAI", ' '"The function of AcrionAI", "What llm AcrionAI support"]'
        )
    elif "sort the remaining search results" in prompt:
        return "[1,2]"
