...
from acrionai.const import DEFAULT_MAX_TOKENS, DEFAULT_TOKEN_SIZE
from acrionai.logs import logger
from acrionai.provider import AcrionAILLM
from acrionai.provider.base_llm import BaseLLM
from acrionai.schema import Message, SimpleMessage
from acrionai.utils.redis import Redis
...
class BrainMemory(BaseModel):
    ...
    async def summarize(self, llm, max_words=200, keep_language: bool = False, limit: int = -1, **kwargs):
        if isinstance(llm, AcrionAILLM):
            return await self._acrionai_summarize(max_words=max_words)
    ...
    async def _acrionai_summarize(self, max_words=200):
        if not self.history:
            return ""
        ...
        return BrainMemory.to_acrionai_history_format(self.history)

    @staticmethod
    def to_acrionai_history_format(history) -> str:
    ...
    async def is_related(self, text1, text2, llm):
        if isinstance(llm, AcrionAILLM):
            return await self._acrionai_is_related(text1=text1, text2=text2, llm=llm)
        return await self._openai_is_related(text1=text1, text2=text2, llm=llm)

    @staticmethod
    async def _acrionai_is_related(**kwargs):
        return False
    ...
    async def rewrite(self, sentence: str, context: str, llm):
        if isinstance(llm, AcrionAILLM):
            return await self._acrionai_rewrite(sentence=sentence, context=context, llm=llm)
        return await self._openai_rewrite(sentence=sentence, context=context, llm=llm)

    @staticmethod
    async def _acrionai_rewrite(sentence: str, **kwargs):
        return sentence
