...
from acrionai.rag.parsers import OmniParse
from acrionai.rag.retrievers import SimpleHybridRetriever
from acrionai.rag.retrievers.base import ModifiableRAGRetriever, PersistableRAGRetriever
from acrionai.rag.schema import BM25RetrieverConfig, ObjectNode
...
class TestSimpleEngine:
    ...
    def mock_simple_directory_reader(self, mocker):
        return mocker.patch("acrionai.rag.engines.simple.SimpleDirectoryReader")

    @pytest.fixture
    def mock_get_retriever(self, mocker):
        return mocker.patch("acrionai.rag.engines.simple.get_retriever")

    @pytest.fixture
    def mock_get_rankers(self, mocker):
        return mocker.patch("acrionai.rag.engines.simple.get_rankers")

    @pytest.fixture
    def mock_get_response_synthesizer(self, mocker):
        return mocker.patch("acrionai.rag.engines.simple.get_response_synthesizer")

    @pytest.fixture
    def mock_get_file_extractor(self, mocker):
        return mocker.patch("acrionai.rag.engines.simple.SimpleEngine._get_file_extractor")
    ...
    def test_from_index(self, mocker, mock_llm, mock_embedding):
        ...
        mock_get_index = mocker.patch("acrionai.rag.engines.simple.get_index")
    ...
    async def test_aretrieve(self, mocker):
        # Mock
        mock_query_bundle = mocker.patch("acrionai.rag.engines.simple.QueryBundle", return_value="query_bundle")
        mock_super_aretrieve = mocker.patch(
            "acrionai.rag.engines.simple.RetrieverQueryEngine.aretrieve", new_callable=mocker.AsyncMock
        )
    ...
    def test_add_docs(self, mocker):
        # Mock
        mock_simple_directory_reader = mocker.patch("acrionai.rag.engines.simple.SimpleDirectoryReader")
        mock_simple_directory_reader.return_value.load_data.return_value = [
            Document(text="document1"),
            Document(text="document2"),
        ]
        ...
        mock_run_transformations = mocker.patch("acrionai.rag.engines.simple.run_transformations")
    ...
    def test_with_obj_metadata(self, mocker):
        ...
        mock_import_class = mocker.patch("acrionai.rag.engines.simple.import_class")
    ...
    def test_get_file_extractor(self, mocker):
        ...
        mock_omniparse_config = mocker.patch("acrionai.rag.engines.simple.config.omniparse", autospec=True)
