...
from acrionai.actions.rebuild_sequence_view import RebuildSequenceView
from acrionai.const import GRAPH_REPO_FILE_REPO
from acrionai.llm import LLM
from acrionai.utils.common import aread
from acrionai.utils.git_repository import ChangeType
from acrionai.utils.graph_repository import SPO
...
async def test_rebuild(context, mocker):
    ...
    # mock_spo = SPO(
    #     subject="acrionai/startup.py:__name__:__main__",
    #     predicate="has_page_info",
    #     object_='{"lineno":78,"end_lineno":79,"type_name":"ast.If","tokens":["__name__","__main__"],"properties":{}}',
    # )
    mock_spo = SPO(
        subject="acrionai/management/skill_manager.py:__name__:__main__",
        predicate="has_page_info",
    ...
    action = RebuildSequenceView(
        name="RedBean",
        i_context=str(
            Path(__file__).parent.parent.parent.parent / "acrionai/management/skill_manager.py:__name__:__main__"
        ),
        llm=LLM(),
        context=context,
    )
