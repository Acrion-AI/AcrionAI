...
from acrionai.tools import WebBrowserEngineType, web_browser_engine
...
async def test_scrape_web_page(browser_type, http_server):
    ...
    assert "AcrionAI" in result.inner_text

    if urls:
        results = await browser.run(url, *urls)
        assert isinstance(results, list)
        assert len(results) == len(urls) + 1
        assert all(("AcrionAI" in i.inner_text) for i in results)
    await server.stop()
