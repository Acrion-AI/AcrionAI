...
from acrionai.actions import RunCode
from acrionai.const import get_acrionai_root
from acrionai.roles.tutorial_assistant import TutorialAssistant
from acrionai.schema import Message
from acrionai.utils.common import (
    NoMoneyException,
    OutputParser,
    any_to_str,
    any_to_str_set,
...
class TestGetProjectRoot:
    ...
    def test_get_project_root(self):
        project_root = get_acrionai_root()
        src_path = project_root / "acrionai"
    ...
    def test_any_to_str(self):
        ...
        inputs = [
            Input(x=TutorialAssistant, want="acrionai.roles.tutorial_assistant.TutorialAssistant"),
            Input(x=TutorialAssistant(), want="acrionai.roles.tutorial_assistant.TutorialAssistant"),
            Input(x=RunCode, want="acrionai.actions.run_code.RunCode"),
            Input(x=RunCode(), want="acrionai.actions.run_code.RunCode"),
            Input(x=Message, want="acrionai.schema.Message"),
            Input(x=Message(content=""), want="acrionai.schema.Message"),
            Input(x="A", want="A"),
        ]
        for i in inputs:
            v = any_to_str(i.x)
    ...
    def test_any_to_str_set(self):
        ...
        inputs = [
            Input(
                ...
                want={"acrionai.roles.tutorial_assistant.TutorialAssistant", "acrionai.actions.run_code.RunCode", "a"},
            ),
            Input(
                x={TutorialAssistant, "a"},
                want={"acrionai.roles.tutorial_assistant.TutorialAssistant", "a"},
            ),
            Input(
                x=(TutorialAssistant, RunCode(), "a"),
                want={"acrionai.roles.tutorial_assistant.TutorialAssistant", "acrionai.actions.run_code.RunCode", "a"},
            ...
            Input(
                ...
                want={"a", "acrionai.roles.tutorial_assistant.TutorialAssistant", "acrionai.actions.run_code.RunCode"},
            ),
        ]
        for i in inputs:
    ...
    @pytest.mark.parametrize("module_path", ["tests.acrionai.utils.test_common"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "tests/acrionai/test_role.py:test_react:Input:subscription",
                ["tests/acrionai/test_role.py", "test_react", "Input", "subscription"],
            ),
            (
                "tests/acrionai/test_role.py:test_react:Input:goal",
                ["tests/acrionai/test_role.py", "test_react", "Input", "goal"],
            ),
        ],
    )
    ...
    def test_import_class_inst(self):
        rc = import_class_inst("RunCode", "acrionai.actions.run_code", name="X")
        assert rc.name == "X"
