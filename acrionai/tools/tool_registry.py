...
from acrionai.const import TOOL_SCHEMA_PATH
from acrionai.logs import logger
from acrionai.tools.tool_convert import (
    convert_code_to_tool_schema,
    convert_code_to_tool_schema_ast,
)
from acrionai.tools.tool_data_type import Tool, ToolSchema
...
def register_tool(tags: list[str] = None, schema_path: str = "", **kwargs):
    ...
    def decorator(cls):
        ...
        if "acrionai" in file_path:
            # split to handle ../acrionai/acrionai/tools/... where only metapgt/tools/... is needed
            file_path = "acrionai" + file_path.split("acrionai")[-1]
