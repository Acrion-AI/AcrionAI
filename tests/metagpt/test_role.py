...
from acrionai.actions import Action, ActionOutput, UserRequirement
from acrionai.environment import Environment
from acrionai.provider.base_llm import BaseLLM
from acrionai.roles import Role
from acrionai.schema import Message
from acrionai.utils.common import any_to_name, any_to_str
...
def test_basic():
    mock_role = MockRole()
    assert mock_role.addresses == ({"tests.acrionai.test_role.MockRole"})
    assert mock_role.rc.watch == {"acrionai.actions.add_requirement.UserRequirement"}

    mock_role = MockRole(name="mock_role")
    assert mock_role.addresses == {"tests.acrionai.test_role.MockRole", "mock_role"}
