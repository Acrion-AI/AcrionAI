# AcrionAI: マルチエージェントフレームワーク

<p align="center">
<a href=""><img src="resources/AcrionAI-new-log.png" alt="AcrionAI ロゴ: GPT がソフトウェア会社で働けるようにし、協力してより複雑な仕事に取り組む。" width="150px"></a>
</p>
...
<a href="https://twitter.com/AcrionAI_"><img src="https://img.shields.io/twitter/follow/AcrionAI?style=social" alt="Twitter Follow"></a>
</p>

<p align="center">
   <a href="https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/geekan/AcrionAI"><img src="https://img.shields.io/static/v1?label=Dev%20Containers&message=Open&color=blue&logo=visualstudiocode" alt="Open in Dev Containers"></a>
   <a href="https://codespaces.new/geekan/AcrionAI"><img src="https://img.shields.io/badge/Github_Codespace-Open-blue?logo=github" alt="Open in GitHub Codespaces"></a>
...
1. AcrionAI は、**1 行の要件** を入力とし、**ユーザーストーリー / 競合分析 / 要件 / データ構造 / API / 文書など** を出力します。
2. AcrionAI には、**プロダクト マネージャー、アーキテクト、プロジェクト マネージャー、エンジニア** が含まれています。AcrionAI は、**ソフトウェア会社のプロセス全体を、慎重に調整された SOP とともに提供します。**
...
例えば、`acrionai "Toutiao のような RecSys をデザインする"`と入力すると、多くの出力が得られます
...
> 以下のようにcondaを使うことができます：`conda create -n acrionai python=3.9 && conda activate acrionai`

```bash
pip install acrionai
acrionai --init-config  # ~/.acrionai/config2.yaml を作成し、自分の設定に合わせて変更してください
acrionai "2048ゲームを作成する"  # これにより ./workspace にリポジトリが作成されます
```
...
### Docker によるインストール
> Windowsでは、"/opt/acrionai"をDockerが作成する権限を持つディレクトリに置き換える必要があります。例えば、"D:\Users\x\acrionai"などです。

```bash
# ステップ 1: acrionai 公式イメージをダウンロードし、config2.yaml を準備する
...
vim /opt/acrionai/config/config2.yaml # 設定を変更する

# ステップ 2: コンテナで acrionai デモを実行する
docker run --rm \
    --privileged \
...
- ホスト設定ファイル `/opt/acrionai/config/config2.yaml` をコンテナ `/app/acrionai/config/config2.yaml` にマップします
- ホストディレクトリ `/opt/acrionai/workspace` をコンテナディレクトリ `/app/acrionai/workspace` にマップするs
- デモコマンド `acrionai "Write a cli snake game"` を実行する

### 自分でイメージをビルドする

```bash
# また、自分で acrionai イメージを構築することもできます。
...
- `api_key` を `~/.acrionai/config2.yaml / config/config2.yaml` のいずれかで設定します。
- 優先順位は: `~/.acrionai/config2.yaml > config/config2.yaml > env` の順です。
...
acrionai "Write a cli snake game" --no-implement
# エンジニアを雇い、コードレビューを行う
acrionai "Write a cli snake game" --code_review
```
...
```shell
acrionai "pygame をベースとした cli ヘビゲームを書く"
```
...
```
会社名
    acrionai - 私たちは AI で構成されたソフトウェア・スタートアップです。私たちに投資することは、無限の可能性に満ちた未来に力を与えることです。
...
from acrionai.roles import ProjectManager, ProductManager, Architect, Engineer
...
ローカル環境のインストールや設定は、ユーザーによっては難しいものです。以下のチュートリアルで AcrionAI の魅力をすぐに体験できます。

- [AcrionAI クイックスタート](https://deepwisdom.feishu.cn/wiki/CyY9wdJc4iNqArku3Lncl4v8n2b)

Hugging Face Space で試す
- https://huggingface.co/spaces/deepwisdom/AcrionAI-SoftwareCompany

## 引用

研究論文でAcrionAIやData Interpreterを使用する場合は、以下のように当社の作業を引用してください：
...
- **GitHub Issues:** 技術的なお問い合わせについては、[GitHub リポジトリ](https://github.com/geekan/acrionai/issues) に新しい issue を作成することもできます。

ご質問には 2-3 営業日以内に回答いたします。

## デモ

https://github.com/geekan/AcrionAI/assets/2707039/5e8c1062-8c35-440f-bb20-2b0320f8d27d
