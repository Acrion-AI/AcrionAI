...
from acrionai.const import GRAPH_REPO_FILE_REPO
from acrionai.logs import logger
from acrionai.repo_parser import CodeBlockInfo, DotClassInfo
...
from acrionai.utils.graph_repository import SPO, GraphKeyword, GraphRepository
...
class RebuildSequenceView(Action):
    ...
    def _get_full_filename(root: str | Path, pathname: str | Path) -> Path | None:
        ...
        Examples:
            If `root`(workdir) is "/User/xxx/github/AcrionAI/acrionai", and the `pathname` is
            "acrionai/management/skill_manager.py", then the returned value will be
            "/User/xxx/github/AcrionAI/acrionai/management/skill_manager.py"
        """
