...
class MockLLM(OriginalLLM):
    ...
    async def original_aask_batch(self, msgs: list, timeout=3) -> str:
        """A copy of acrionai.provider.base_llm.BaseLLM.aask_batch, we can't use super().aask because it will be mocked"""
        context = []
        for msg in msgs:
    ...
    async def original_aask_code(self, messages: Union[str, Message, list[dict]], **kwargs) -> dict:
        """
        A copy of acrionai.provider.openai_api.OpenAILLM.aask_code, we can't use super().aask because it will be mocked.
