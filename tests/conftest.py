...
from acrionai.const import DEFAULT_WORKSPACE_ROOT, TEST_DATA_PATH
from acrionai.context import Context as AcrionAIContext
from acrionai.llm import LLM
from acrionai.logs import logger
from acrionai.utils.git_repository import GitRepository
...
def llm_mock(rsp_cache, mocker, request):
    ...
    mocker.patch("acrionai.provider.base_llm.BaseLLM.aask", llm.aask)
    mocker.patch("acrionai.provider.base_llm.BaseLLM.aask_batch", llm.aask_batch)
    mocker.patch("acrionai.provider.openai_api.OpenAILLM.aask_code", llm.aask_code)
    yield mocker
...
def new_filename(mocker):
    ...
    mocker.patch("acrionai.utils.file_repository.FileRepository.new_filename", lambda: "20240101")
    yield mocker
...
def http_server():
    async def handler(request):
        return aiohttp.web.Response(
            ...
            <title>AcrionAI</title></head><body><h1>AcrionAI</h1></body></html>""",
